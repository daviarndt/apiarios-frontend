...
function createData(id, nome, cidade) {
    return { id, nome, cidade };
}

function populateRows(response) {
    response.data.dados.forEach((element) => {
        rows.push(createData(element.id, element.nome, element.cidade))
    })
    cache.set('table', rows)
}

function callPopulate() {
    if (cache.get('table') == undefined) {
        Api.buscarApiariosPorUsuario(6).then(
            populateRows,
            console.log('Não foi possivel popular a tabela')
        )
    } else {
